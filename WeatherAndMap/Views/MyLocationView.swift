...
struct MyLocationView: View {
    // ロケーションマネージャーは前の画面のものを受け取って使い回す
    ...
    var body: some View {
        ...
        MapReader { mapProxy in
            Map(position: $position) { // Mapの表示位置を$positionに指定
                UserAnnotation() // ユーザー位置の青ピンを表示
            ...
            .mapStyle(.standard) // 地図のスタイル。.hybrid,.imageryでもOK
            .mapControls {
                MapUserLocationButton() // ユーザー位置に戻るボタン
                MapCompass() // コンパス
                MapScaleView() // 縮尺表示
            }
            // 地図がタップされたときの処理、locationは画面上のタップされた点の座標
